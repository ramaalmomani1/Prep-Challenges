...
const filterLinks = (str) => {
    let linkReg = /[www]+.+(\w+)+(.com|.org|.net)/ig
    let match =str.match(linkReg);
    let arrayToString =match.toString();
...
if (str=== ' '){
    ...
    let lowerCase = str.match (/[a-z0-9]+/ig).join('').toLowerCase()
    let b   =lowerCase.split('').reverse().join('')
    
    if (lowerCase===b)   return true;
return false
