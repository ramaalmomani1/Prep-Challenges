...
const cvsFiltration = (arr) => {
 let filteredArr =   arr.filter(item => item.yearsOfExperience> 4 && item.tech === 'JS'  )

 let finalResult = filteredArr.map(item => 
(  { fullName: `${item.firstName || ''} ${item.LastName || ''}`.trim() , tech: item.tech})
...
const vowelsFiltration = (arr) => {
    let regex = /[aeiou]/i
  let filteredArr = arr.filter(word => !regex.test(word));
    return filteredArr
